...
that I enjoy coding in this language. Since I have no expectations
regarding anyone contributing to this project, I didn't create a repository for it.
Instead, I included it in this blog's repo. You can take a look at the individual files below
or access the project's root [here](https://github.com/OmarMesqq/omarmesqq.github.io/tree/main/projects/graphs).
...
The first two `graph.h` and `graph.c` files correspond to declarations and definitions of the graph API which will
be used for [testing](https://en.wikipedia.org/wiki/Test-driven_development) in `main.c`, 
whereas `queue.h` and `queue.c` contain the declarations and definitions of 
a simple [queue](https://en.wikipedia.org/wiki/Queue_(abstract_data_type)) used as an auxiliary data structure in
...
{% highlight c %}
{% include_absolute projects/graphs/graph.h, 4-14 %}
{% endhighlight %}
...
{% highlight c %}
{% include_absolute projects/graphs/graph.h, 16-20 %}
{% endhighlight %}
...
{% highlight c %}
{% include_absolute projects/graphs/graph.h, 22-28 %}
{% endhighlight %}
...
{% highlight c %}
{% include_absolute projects/graphs/queue.h, 5-13 %}
{% endhighlight %}
...
{% highlight c %}
{% include_absolute projects/graphs/queue.h, 16-20 %}
{% endhighlight %}
...
[ethos](https://en.wikipedia.org/wiki/Ethos) of continuously improving oneself.
...
by analyzing `main.c`. I made this assignment from a top down perspective through test-driven development,
