...
In order to achieve this, clever people came up with two main ways of *describing* graphs:
...
it's simpler to implement, and grapples the multidimensionality of this data structure fairly well.
...
with an undirected graph. The conditional only stores edges $$ (u, v) $$ where $$ u < v $$. This is design matter, since
...
performance as much as the former step. I am not able to mathematically justify the following, as I lack
knowledge, space, and time, but [generally speaking, Kruskal's algorithm should run in $$O(E\;log\;E)$$ time](https://en.wikipedia.org/wiki/Kruskal%27s_algorithm#Complexity).

At the end of the main loop, auxiliary data structures are freed and the newly discovered MST is returned to the caller:
{% highlight c %}
...
Originally, the purpose of this post was to reinforce my knowledge and applied work on graphs and disjoint sets,
as my memory can get [brittle over time](https://en.wikipedia.org/wiki/Forgetting_curve).

However, as I wrote the article, I realized it can be the commence of an interesting, fruitful, and long-lasting
blogging journey (hopefully). I also managed to resignify the true prupose of it: to **share knowledge with
other people and the world**. This matters a lot to me. So, once again, I am asking for [feedback](mailto:omarmsqt@gmail.com) 
on my writing: this wasn't supposed to teach you everything on graphs, disjoint sets, and Kruskal's. 
